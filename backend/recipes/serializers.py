...
from api.validators import (validate_ids_not_null_unique_collection,
...
validate_recipe_picture = partial(
    validate_image, max_file_size=stgs.MAX_FILE_SIZE
)
...
class IngredientRecipeSerializer(ss.Serializer):
    ...
    def get_ingredient(self, ingredient_id):
        ...
        try:
            ...
            raise ValidationError(Warn.OBJECT_NOT_FOUND)
    ...
    def create(self, validated_data):
        ...
        try:
            ...
            if not recipe:
                raise ValidationError(Warn.RECIPE_CONTEXT_MISSING)
...
class TagsWriteSerializer(TagsReadSerializer):
    ...
    def to_internal_value(self, data):
        ...
        if not isinstance(data, int):
            raise ValidationError(Warn.TAG_ID_REQUIRED)
        return {'id': data}
...
class BaseRecipeSerializer(ss.ModelSerializer):
    ...
    name = ss.CharField(
        validators=[
            MinLengthValidator(
                stgs.RECIPE_MIN_LENGTH, message=Warn.NAME_LEN_MIN_REQUIRED
            ),
            MaxLengthValidator(
                stgs.RECIPE_MAX_LENGTH, message=Warn.NAME_LEN_MAX_REQUIRED
            )
        ],
...
class RecipesSerializer(BaseRecipeSerializer):
    ...
    def validate_ingredients(self, ingredients):
        ...
        for ingredient in ingredients:
            validate_amount_interval(ingredient['amount'])
        return validate_ids_not_null_unique_collection(
            values=ingredients,
    ...
    class Meta(BaseRelationSerializer.Meta):
        ...
        validators = [
            UniqueTogetherValidator(
                ...
                fields=('user', 'recipe'),
                message=Warn.RECIPE_IN_SHOPPING_CART_EXISTS
            ),
        ]
    ...
    class Meta(BaseRelationSerializer.Meta):
        ...
        validators = [
            UniqueTogetherValidator(
                ...
                fields=('user', 'recipe'),
                message=Warn.RECIPE_IN_FAVORITE_EXISTS
            ),
        ]
